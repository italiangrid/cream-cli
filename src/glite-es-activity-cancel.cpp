...
int main ( int argc, char* argv[] ) {
  ...
  desc.add_options()
    ...
    (
     ...
     "The activity identifier(s) to cancel"
     )
    (
     "endpoint,e", 
     po::value<string>(&endpoint), 
     "Set the endpoint to send the activity cancel request"
     )
    (
     "epr,E",
     po::value<string>(&EPR),
    ...
    (
     ...
     "Set the input file where to find the Activity identifiers to cancel"
     );
  ...
  if(vm.count("help"))
    {
      cout << endl << es_help_messages::VersionID 
	   ...
	   << endl
	   << endl
	   << "Usage: " << es_help_messages::cancel_usage
	   << es_help_messages::common_opts
	   << es_help_messages::cancel_custom_opts
	   << endl;
      return 0;
    }
   ...
   */
  string error;
  if(!vm.count("certfile") && !vm.count("epr")) {
  ...
  AbstractCall* call = CallFactory::makeCall( endpoint, aIDs, AbstractCall::CANCEL );
  ...
  if(debug)
    cout << "Sending ActivityCancel request for ActivityID(s) {" 
	 << join(aIDs, ", ") 
	 << "} to service [" 
	 << endpoint
	 << "]" 
