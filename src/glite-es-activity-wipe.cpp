...
int main ( int argc, char* argv[] ) {
  ...
  desc.add_options()
    ...
    (
     ...
     "The activity identifier(s) to wipe"
     )
    (
     "endpoint,e", 
     po::value<string>(&endpoint), 
     "Set the endpoint to send the activity wipe request"
     )
    (
     "epr,E",
     po::value<string>(&EPR),
    ...
    (
     ...
     "Set the input file where to find the Activity identifiers to wipe"
     );
  ...
  if(vm.count("help"))
    {
      cout << endl << es_help_messages::VersionID 
	   ...
	   << endl
	   << endl
	   << "Usage: " << es_help_messages::wipe_usage
	   << es_help_messages::common_opts
	   << es_help_messages::wipe_custom_opts
	   << endl;
      return 0;
    }
  ...
  AbstractCall* call = CallFactory::makeCall( endpoint, aIDs, AbstractCall::WIPE );
  ...
  if(debug)
    cout << "Sending ActivityWipe request for ActivityID(s) {" 
	 << join(aIDs, ", ") 
	 << "} to service [" 
	 << endpoint
	 << "]" 
