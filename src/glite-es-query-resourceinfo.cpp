...
int main( int argc, char* argv[] ) {
  ...
  desc.add_options()
    ...
    ("dialect,D", po::value<string>(&DIALECT), "Specify the query dialect")
    ("query,Q", po::value<string>(&QUERY), "Specify the query");
  ...
  if(vm.count("help"))
    {
      cout << endl << es_help_messages::VersionID 
           ...
           << endl
           << endl
           << "Usage: " << es_help_messages::queryresourceinfo_usage
           << es_help_messages::common_opts
           << es_help_messages::queryresourceinfo_custom_opts
           << endl;
      return 0;
    }
  ...
  if(vm.count("conf") && !vm.count("epr")) {
    ...
    if(!op) {
      ...
      customepr = op->get_epr( "queryresourceinfo" );
      delete op;
    }
  }
   ...
   *
   */
  AbstractCall *call = CallFactory::makeCall( deleg_service, 1, DIALECT, QUERY );
    
  if(debug)
    cout << "Querying ResourceInfo to service [" << deleg_service << "]" << endl;
  ...
  cout << "SOAP RESPONSE=[" << endl <<call-> get_soap_buffer( ) << endl << "]" << endl;
