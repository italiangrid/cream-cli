...
int main ( int argc, char* argv[] ) {
  ...
  desc.add_options()
    ...
    (
     ...
     "The activity identifier(s) to query the status of"
     )
    (
     "endpoint,e", 
  ...
  if(vm.count("help"))
    {
      cout << endl << es_help_messages::VersionID 
	   ...
	   << endl
	   << endl
	   << "Usage: " << es_help_messages::status_usage
	   << es_help_messages::common_opts
	   << es_help_messages::status_custom_opts
	   << endl;
      return 0;
    }
  ...
  AbstractCall* call = CallFactory::makeCall( endpoint, aIDs, AbstractCall::STATUS );
  ...
  if(debug)
    cout << "Sending ActivityStatus request for ActivityID(s) {" 
	 << join(aIDs, ", ") 
	 << "} to service [" 
	 << endpoint
	 << "]" 
  ...
  map<string, ActivityStatus> good;
  map<string, InternalBaseFault> bad;
  
  call->getResponse( good, bad );
  
  map<string, ActivityStatus>::const_iterator git = good.begin();
  map<string, InternalBaseFault>::const_iterator bit = bad.begin();
  
  for( ; git != good.end(); ++git )
    {
      cout << "*****************************************\n";
      cout << "ActivityID  = " << git->first<<endl;
      cout << "Status      = " << git->second.getStatusString() << endl;
      vector<string> attrs; 
      git->second.getStatusAttributesString(attrs);
      cout << "Attributes  = {" << join(attrs, ", ") << "}" << endl;
      cout << "Timestamp   = " << git->second.getTimestampString( ) << endl;
      cout << "Description = " << git->second.getDescription( ) << endl;
    }
  
  for( ; bit != bad.end(); ++bit )
    {
      cout << "*****************************************\n";
      cout << "ActivityID  = " << bit->first<<endl;//getActivityID()<<endl;
      cout << "Message     = " << bit->second.Message<<endl;
      cout << "Timestamp   = " << (bit->second.Timestamp ? time_to_string(*bit->second.Timestamp) : "N/A")<< endl;
      cout << "Description = " << (bit->second.Description ? *bit->second.Description : "N/A") << endl;
      cout << "FailCode    = " << (bit->second.FailureCode ? boost::lexical_cast<string>(*bit->second.FailureCode) : "N/A" )<< endl;
    }
