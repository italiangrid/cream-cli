 ...
 *
 */
#include "services/cli_service_allowed_submission.h"
...
//  bool noint = false;
  ...
  glite::ce::cream_cli::services::cli_service_allowed_submission AS( opt );
  
  if(AS.execute( ) ) {
    AS.getLogger()->fatal( AS.get_execution_error_message( ) );
    return 1;
  }
  
  if( AS.is_submission_allowed() )
     cout << "Job Submission to this CREAM CE is enabled" << endl;
   else
     cout << "Job Submission to this CREAM CE is disabled" << endl;
...
//       {0, 0, 0, 0}
//     };
//     c = getopt_long(argc, argv, "nhdvAr:l:c:p:t:", long_options, &option_index);
//     if ( c == -1 )
//       break;
// 
//     switch(c) {
...
//   if(debug || redir_out) {
//       if(!redir_out) {
//           logfile = logger_instance->getLogFileName(confMgr.getProperty("ALLOWEDSUB_LOG_DIR", DEFAULT_LOG_DIR).c_str(), "glite-ce-allowed-submission");
//       }
...
//       {
// 	log_dev->error( "Endpoint not specified in the right format: should be <host>[:tcpport]; tcpport must be a positive number <= 65535. Stop.");
// 	exit(1);
//       }
...
//   //bool resp = false;
// 
//   ServiceInfoWrapper info;
//   AbsCreamProxy* creamClient = CreamProxyFactory::make_CreamProxyServiceInfo( &info, 2, timeout );
// 
//   if(!creamClient) 
//     {
//       logger_instance->log(log4cpp::Priority::FATAL, 
...
// 
//   try {
//     //string p =serviceAddress.c_str();
//     //resp = initSoap->DoesAcceptJobSubmissions( serviceAddress.c_str() );
//     
//     creamClient->execute( serviceAddress );
// 
//     //resp = initSoap->DisableAcceptJobSubmissions( serviceAddress.c_str() );
//   } catch(BaseException& ex) {
//       log_dev->log(log4cpp::Priority::ERROR, ex.what(), true, true, true);
//       exit(1);
//   } catch(exception& ex) {
//     log_dev->log(log4cpp::Priority::ERROR, ex.what(), true, true, true);
//     exit(1);
//   }
// 
//   if( info.getAcceptJobSubmission() )
//     printf("Job Submission to this CREAM CE is enabled\n");
//   else
//     printf("Job Submission to this CREAM CE is disabled\n");
// 
//   //delete(initSoap);
//   return 0;
}
...
void printhelp() {
  printf("%s\n",help_messages::getDoesSubHelp().c_str());
}
