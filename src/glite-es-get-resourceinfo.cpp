...
int main( int argc, char* argv[] ) {
  ...
  int     timeout;
  bool    debug = false;
  string  EPR = "/ce-cream-es/services/ResourceInfoService";
  ...
  if(vm.count("help"))
    {
      cout << endl << es_help_messages::VersionID 
           ...
           << endl
           << endl
           << "Usage: " << es_help_messages::getresourceinfo_usage
           << es_help_messages::common_opts
           << es_help_messages::getresourceinfo_custom_opts
           << endl;
      return 0;
    }
  ...
  if(vm.count("conf") && !vm.count("epr")) {
    ...
    if(!op) {
      ...
      customepr = op->get_epr( "getresourceinfo" );
      delete op;
    }
  }
   ...
   *
   */
  AbstractCall *call = CallFactory::makeCall( deleg_service, 0 );
    
  if(debug)
    cout << "Asking ResourceInfo to service [" << deleg_service << "]" << endl;
  ...
  string soap_buf( call->get_soap_buffer( ) );
  size_t pos_body_start = soap_buf.find("<soap:Body>");
  size_t pos_body_end   = soap_buf.find("</soap:Body>");
  string sub_buf = soap_buf.substr( pos_body_start+11, pos_body_end-11-pos_body_start );
