...
#include "services/cli_service_jobsuspend.h"
#include "hmessages.h"
...
int main(int argc, char *argv[]) {
  ...
  string joblist_file = "";
  bool nomsg = false;
  vector<string> all_jobids_to_suspend, all_jobids_to_actually_suspend;
  ...
  desc.add_options()
    ...
    (
     "jobid,j", po::value<vector<string> >(&all_jobids_to_suspend), ""
     )
    (
     "noint,N", ""
     )
  ...
  if( ( vm.count("all") && vm.count("input") ) || ( !all_jobids_to_suspend.empty() && vm.count("all") )) {
    // ERROR
    cerr << "--all and --input or --all and specification of JobID(s) as argument are mutually exclusive. Stop."<<endl;
    return 1;
  }
      
  if( (all_jobids_to_suspend.empty() && !vm.count("input")) && !vm.count("all")) {
    //ERROR
  ...
  glite::ce::cream_cli::services::cli_service_jobsuspend JS( opt, &result );
  
  if( vm.count("input") )
    JS.set_inputfile( joblist_file );
    
  if( vm.count("jobid") ) {
    vector<string>::iterator it = all_jobids_to_suspend.begin( );
    for( ; it != all_jobids_to_suspend.end(); ++it )
    {
      JS.insert_job_to_suspend( *it );
    }
  }
  ...
  for( map<string, string>::iterator it = result.begin();
       ...
       ++it )
  {
    cout << "Couldn't suspend job [" << it->first << "]: " << it->second << endl;
  }
...
void printhelp() {
  printf("%s\n",help_messages::getSuspendHelp().c_str());
}
