#include "cli_service_jobresume.h"
...
cli_service_jobresume::cli_service_jobresume( const cli_service_common_options& opt, map<string, string>* target )
 : cli_service( opt ), m_jobs_to_resume( vector<string>() ), m_inputfile( "" ), m_resumeall( false ), m_fail_result( target ),
 m_joblist_from_file( false )
{
}

//______________________________________________________________________________
cli_service_jobresume::~cli_service_jobresume( ) throw( )
{
}

//______________________________________________________________________________
int cli_service_jobresume::execute( void ) throw( )
{
  string VO = "", errmex = "";
  long int i;
  ...
  this->set_logfile( "RESUME_LOG_DIR", "/tmp/glite_cream_cli_logs", "glite-ce-job-resume" );
  ...
  vector<string> jobnumbers_to_resume;
  bool resume_all_jobs_from_file = false;
  
  if(m_joblist_from_file) {
    
    if(!cliUtils::interactiveChoice("Resume", 
    				    m_inputfile.c_str(), 
				    m_noint, 
				    m_debug, 
				    m_nomsg, 
				    resume_all_jobs_from_file, 
				    jobnumbers_to_resume,
				    m_jobs_to_resume, 
  ...
  vector<string> all_jobids_to_actually_resume;
  map <string, vector<string> > url_jobids;
  
  if(!m_resumeall) {
    char confirm;
    if(!m_noint) {
      printf("\nAre you sure you want to resume specified job(s) [y/n]: ");
      cin >> confirm;
    } else confirm ='y';

    if(confirm != 'y') {
      printf("Resume aborted. Bye.\n");
      m_execution_fail_message="";
      return 1;
    }
     ...
     *
     */
    for(jcounter=0; jcounter<m_jobs_to_resume.size(); jcounter++) {
      if(m_joblist_from_file) {
        if(!resume_all_jobs_from_file) {
	  bool isthere = false;
	  for(
	      vector<string>::iterator it = jobnumbers_to_resume.begin();
	      it!=jobnumbers_to_resume.end();
	      it++)
	    if( jcounter == (unsigned int)atoi( (*it).c_str()) ) { isthere = true; break; }
	  if(!isthere) continue;
        }
//	if(m_logging)
	  this->getLogger()->debug( string("Will resume job [")+m_jobs_to_resume.at(jcounter)+"]" );
	  
	all_jobids_to_actually_resume.push_back(m_jobs_to_resume.at(jcounter));
      }
      
      else all_jobids_to_actually_resume.push_back( m_jobs_to_resume.at(jcounter) );
    ...
    for(vector<string>::const_iterator jit = all_jobids_to_actually_resume.begin();
	jit != all_jobids_to_actually_resume.end();
...
//	  }
  ...
  try {
    ...
    if(m_resumeall && !m_noint) {
      char confirm;
      printf("\nGoing to resume all job from CE [%s]. Do you want to proceed [y/n]? ",
    ...
    if(m_resumeall) {
//      if(m_logging)
        this->getLogger()->debug( string("Resume all jobs on [")+serviceAddress+"]..." );
	
      wr = new apiproxy::JobFilterWrapper(vector<apiproxy::JobIdWrapper>(), vector<string>(), -1, -1, "", "");
      
      m_creamClient = apiproxy::CreamProxyFactory::make_CreamProxyResume( wr, &result, m_soap_timeout );
