...
int main ( int argc, char* argv[] ) {
  ...
  desc.add_options()
    ...
    (
     ...
     "The activity identifier(s) to restart"
     )
    (
     "endpoint,e", 
     po::value<string>(&endpoint), 
     "Set the endpoint to send the activity restart request"
     )
    (
     "epr,E",
     po::value<string>(&EPR),
    ...
    (
     ...
     "Set the input file where to find the Activity identifiers to restart"
     );
  ...
  if(vm.count("help"))
    {
      cout << endl << es_help_messages::VersionID 
	   ...
	   << endl
	   << endl
	   << "Usage: " << es_help_messages::restart_usage
	   << es_help_messages::common_opts
	   << es_help_messages::restart_custom_opts
	   << endl;
      return 0;
    }
  ...
  vector<string> IDS;
  if(!inputFile.empty()) {
    if(!check_input_format(inputFile, keyfile, IDS, error)) {
  ...
  AbstractCall* call = CallFactory::makeCall( endpoint, aIDs, AbstractCall::RESTART );
  
  if ( !::getenv("ES_CLI_NOAUTH") ) {
    if(!call->init( certfile, certfile, error ))
      {
	cerr << error << endl;
	return 1;
      }
  ...
  if(debug)
    cout << "Sending ActivityRestart request for ActivityID(s) {" 
	 << join(aIDs, ", ") 
	 << "} to service [" 
	 << endpoint
	 << "]" 
